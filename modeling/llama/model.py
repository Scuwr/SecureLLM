...
class Attention(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        """
        super().__init__()
        # 70B: n_heads=64, n_kv_heads=8, world_size=8, dim=8192
        self.n_kv_heads = args.n_heads if args.n_kv_heads is None else args.n_kv_heads
        model_parallel_size = fs_init.get_model_parallel_world_size()
        self.n_local_heads = args.n_heads // model_parallel_size
        self.n_local_kv_heads = self.n_kv_heads // model_parallel_size
        self.n_rep = self.n_local_heads // self.n_local_kv_heads
        self.head_dim = args.dim // args.n_heads  # 70B: 128
        ...
        self.adapter_gate = torch.nn.Parameter(torch.zeros(1, args.n_heads, 1, 1))
    ...
    def forward(
        ...
        if self.use_cache:
            ...
            assert start_pos == 0, "start_pos must be 0 if not using cache"
            keys = xk
            values = xv
        ...
        if adapter is not None:
            ...
            adapter_v = repeat_kv(adapter_v, self.n_rep)  # (bs, adapter_len, n_local_heads, head_dim)
        ...
        if adapter is not None:
            ...
            adapter_scores = self.adapter_gate[
                :, self.head_start : self.head_end
            ].tanh().half() * F.softmax(adapter_scores.float(), dim=-1).type_as(xq)
...
class Transformer(nn.Module):
    def __init__(self, params: ModelArgs):
        ...
        self.use_adapter = params.use_adapter
        if self.use_adapter:
    ...
    def forward(self, tokens: torch.Tensor, start_pos: int=0):
        ...
        if self.use_adapter:
            ...
            prompt = [None] * (len(self.layers) - self.params.adapter_layer) + [i for i in prompt]
        else:
            prompt = [None] * len(self.layers)
        mask = None
        if seqlen > 1:
            mask = torch.full(
        ...
        for i, layer in enumerate(self.layers):
            h = layer(h, start_pos, freqs_cis, mask, prompt[i])
        h = self.norm(h)
